...
#
### Having RODAN (https://github.com/biodlab/RODAN/blob/6497c32b57a4a331dc5b75b8a7103e778e2c78aa/accuracy.py)
### model, augmentation on the data was performed to test if accuracy of the model
### can be outperformed. Here, SpecAugment (http://dx.doi.org/10.21437/Interspeech.2019-2680) will be implemented.
...
def train(config = None, args = None, arch = None):
    ...
    for epoch in range(config.epochs):
        ...
        for i, (event, event_len, label, label_len) in enumerate(data_loader):
            ...
            if i % 20 == 0:
                # SpecAugmented implementation
                spec = spectogram.time_masking(event)

                # Invert spectrogram to vector 
                event = spectogram.inv(spec)
    ...
    metrics = {'Train Loss': trainloss, 'Valid Loss': validloss}
    metrics = pd.DataFrame(metrics)
    metrics_csv = metrics.to_csv('results/metrics_csv/loss_aug_spec.csv')

    print("csv file", metrics, "saved")
